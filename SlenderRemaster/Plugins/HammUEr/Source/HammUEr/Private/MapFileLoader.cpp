// HammUEr
// Copyright 2015-2019 Turfster / NT Entertainment
// All Rights Reserved.
...
void MapFileLoader::DoFuncBrush(int32 entityIndex, TArray<UMaterialInterface*> newMaterials)
{
	...
	if (BPtoUse)  // we want to use the blueprint version
	{
		...
		if (myMesh == NULL)
			myMesh = GenerateTemporaryModel(UTF8_TO_TCHAR(data->modelname.c_str()));
...
void MapFileLoader::DoPropStatic(int32 entityIndex, MAPPropStatic* instancedProp)
{
	...
	if (data->modelname != "error")
	{
		...
		FString modelName = UTF8_TO_TCHAR(data->modelname.c_str());
		UStaticMesh* toSpawn = HammUErSettings::FindInStaticMeshDatabase(modelName);
		if (HammUErSettings::placeUnknownModels || (toSpawn != NULL))
		{
			if (toSpawn == NULL)
				toSpawn = GenerateTemporaryModel(modelName);
			FActorSpawnParameters FAS;
			FString tempName = data->name.c_str();
			FString tag = "Entity_" + FString::FromInt(data->id);
...
void MapFileLoader::DoPropDynamic(int32 entityIndex, MAPPropDynamic* instancedProp)
{
	...
	if (data->modelname != "error")
	{
		...
		{
			if (toSpawn == NULL)
				toSpawn = GenerateTemporaryModel(UTF8_TO_TCHAR(data->modelname.c_str()));
			FActorSpawnParameters FAS;
			FString tempName = data->name.c_str();
			FString tag = "Entity_" + FString::FromInt(data->id);
...
void MapFileLoader::ImportIntoWorld(TArray<UMaterialInterface*> newMaterials, bool doAll)
{
	...
	{
		FScopedSlowTask SlowTask2(numEntities, FText::FromString("Importing entities..."));
		SlowTask.MakeDialog();
		...
		*/
		...
		// okay now walk through the paths and hook them up
			for (int j = 0; j < pathNodeList.Num(); j++)
			{
				...
				if (target != NULL)
				{
					pathNodeList[j]->nextTarget = target;
				}
			}
		}
	}
...
UStaticMesh* MapFileLoader::GenerateTemporaryModel(FString modelLocation, bool generateExclamation)
{
	MapBrush* newTempBrush = new MapBrush();
	if (modelLocation.Contains(".mdl"))
		modelLocation = modelLocation.Replace(TEXT(".mdl"), TEXT(""));
	newTempBrush->brushName = TCHAR_TO_UTF8(*modelLocation);
